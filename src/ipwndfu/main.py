...
def xploit():
    ...
    if device.config.cpid != '8920':
        print('This is not a compatible device. alloc8 exploit is for iPhone 3GS only.', file=sys.stderr)
        exit(1)

    if device.config.version == '359.3':
        print('WARNING: iPhone 3GS (old bootrom) was detected. Use 24Kpwn exploit for faster '
              'boots, alloc8 exploit is for testing purposes only.', file=sys.stderr)
    ...
    for byte in nor_data.parts[1]:
        if byte != '\x00':
            print('ERROR: Bytes following IMG2 header in NOR are not zero. alloc8 '
                  'exploit was likely previously installed. Exiting.', file=sys.stderr)
            exit(1)
...
def demote(device=None):
    ...
    if 'PWND:[checkm8]' in serial_number:
        ...
        print('ERROR: Demotion is only supported on devices pwned with checkm8 exploit.', file=sys.stderr)
        exit(1)
...
def dump(device=None, dump_args='', match=None):
    ...
    arg = dump_args
    if arg.count(',') != 1:
        print('ERROR: You must provide exactly 2 comma separated values: address,length', file=sys.stderr)
        exit(1)
...
def dump_rom(device=None):
    ...
    if 'PWND:[checkm8]' in serial_number:
        ...
        if hashlib.sha1(securerom).hexdigest() != pwned.platform.rom_sha1:
            ...
            print('ERROR: SecureROM was dumped, but the SHA1 hash does not match. Exiting.', file=sys.stderr)
            exit(1)
...
def boot(device=None):
    ...
    if 'CPID:8015' not in serial_number or 'PWND:[checkm8]' not in serial_number:
        print(serial_number)
        print('ERROR: Option --boot is currently only supported on iPhone X pwned with checkm8.', file=sys.stderr)
    else:
...
def dump_nor(arg):
    device = PwnedDFUDevice()
    if device.config.cpid != '8920':
        print('This is not a compatible device. Dumping NOR is only supported on iPhone 3GS.', file=sys.stderr)
        exit(1)
...
def flash_nor(arg):
    ...
    f.close()
    if new_nor[:4] != 'IMG2'[::-1]:
        print('ERROR: Bad IMG2 header magic. This is not a valid NOR. Exiting.', file=sys.stderr)
        exit(1)

    device = PwnedDFUDevice()
    if device.config.cpid != '8920':
        print('This is not a compatible device. Flashing NOR is only supported on iPhone 3GS.', file=sys.stderr)
        exit(1)
...
def do_24kpwn(device=None):
    print('*** based on 24Kpwn exploit (segment overflow) by chronic, CPICH, ius, MuscleNerd, '
    ...
    for byte in nor_data.parts[1]:
        if byte != '\x00':
            print('ERROR: Bytes following IMG2 header in NOR are not zero. '
                  'alloc8 exploit was likely previously installed. Exiting.', file=sys.stderr)
            exit(1)

    if len(nor_data.images) == 0:
        print('ERROR: 24Kpwn exploit cannot be installed, because NOR has no valid LLB. Exiting.', file=sys.stderr)
        exit(1)

    # Remove existing 24Kpwn exploit.
    if len(nor_data.images[0]) > 0x24000:
        nor_data.images[0] = image3_24Kpwn.remove_exploit(nor_data.images[0])
    nor_data.images[0] = image3_24Kpwn.exploit(nor_data.images[0], device.securerom_dump())
...
def remove_24kpwn(device=None):
    if not device:
        device = PwnedDFUDevice()
    if device.config.cpid != '8920':
        print('This is not a compatible device. 24Kpwn exploit is only supported on iPhone 3GS.', file=sys.stderr)
        exit(1)

    print('WARNING: This feature is for researchers only. Device will probably not boot into '
    ...
    if len(nor_data.images) == 0:
        print('ERROR: NOR has no valid LLB. It seems that 24Kpwn exploit is not installed. Exiting.', file=sys.stderr)
        exit(1)
    if len(nor_data.images[0]) <= 0x24000:
        print('ERROR: LLB is not oversized. It seems that 24Kpwn exploit is not installed. Exiting.', file=sys.stderr)
        exit(1)
...
def remove_alloc8(device=None):
    ...
    print('WARNING: This feature is for researchers only. Device will probably not '
          'boot into iOS until it is restored in iTunes.', file=sys.stderr)
    input("Press ENTER to continue.")
