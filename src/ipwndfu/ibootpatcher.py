...
def apply_tcr_el3_patch(binary):
    for i in range(0, len(binary), 4):
        if binary[i:i + 4] == MSR_TCR_EL3_X0:
            ...
            binary = binary[:0x1EC] + ORR_X0_X0_0x10000000 + ORR_X0_X0_0x800000 + MSR_TCR_EL1_X0 + ISB + RET + binary[
                                                                                                               0x200:]

            print
            'TCR_EL3 patch: 0x%x' % i
            return binary

    print
    'ERROR: Could not find MSR TCR_EL3, X0 instruction.'
    sys.exit(1)
